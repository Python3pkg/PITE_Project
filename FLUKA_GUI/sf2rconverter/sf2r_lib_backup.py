...
class plot_3d(object):
    ...
    def __plot_3d(self):
        ...
        ## --------------- nxbins, nybins, nzbins = nrbins * npbins, nrbins * npbins, nrbins * npbins
        
        for pentry in range( npbins ):
            for rentry in range( nrbins ):
                r   = hdata[ pos_cnt ]
                phi = FirstP + float( pentry ) * ResP
                xPos[pos_cnt] = r * np.cos(phi)
                yPos[pos_cnt] = r * np.sin(phi)
                #zPos[pos_cnt] = FirstZ # not needed
                pos_cnt += 1
        
        data_to_fill = np.column_stack([zPos, yPos, xPos])
        for __dataset in data_to_fill:
            # print __dataset[0], __dataset[1], __dataset[2]
            self.__histo.Fill( __dataset[0], __dataset[1], __dataset[2])
